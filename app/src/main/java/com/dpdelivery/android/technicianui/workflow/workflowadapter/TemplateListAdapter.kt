...
class TemplateListAdapter(var context: Context, var adapterClickListener: IAdapterClickListener? = null, val stepMap: MutableMap<String, String>, val stepsFinished: MutableMap<String, Boolean>, var submissionField: String, var activationElementId: Int, var syncElementId: Int) : RecyclerView.Adapter<TemplateListAdapter.TemplateListViewHolder>() {

    private var list: ArrayList<WorkFlowDataRes.WorkFlowDataResBody.Step.Template>? = null

    fun addList(list: ArrayList<WorkFlowDataRes.WorkFlowDataResBody.Step.Template>?, submissionField: String, activationElementId: Int, syncElementId: Int) {
        this.list = list
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TemplateListViewHolder {
        ...
        return TemplateListViewHolder(view, adapterClickListener, stepMap, stepsFinished, submissionField, activationElementId, syncElementId)
    }
    ...
    class TemplateListViewHolder(override val containerView: View, var adapterClickListener: IAdapterClickListener? = null, val stepMap: MutableMap<String, String>, val stepsFinished: MutableMap<String, Boolean>, val submissionField: String, val activationElementId: Int, val syncElementId: Int) : RecyclerView.ViewHolder(containerView), LayoutContainer {
        fun bind(context: Context, item: Any, pos: Int) {
            if (item is WorkFlowDataRes.WorkFlowDataResBody.Step.Template) {
                ...
                val adapterLowerList = ElementListAdapter(context, adapterClickListener, stepMap = stepMap, stepsFinished = stepsFinished, submissionField = submissionField, activationElementId = activationElementId, syncElementId = syncElementId)
