...
class JobsListPresenter @Inject constructor(
    ...
    override fun getJobsList(status: String) {
        view?.showProgress()
        subscription.add(
                apiService.getJobsList(CommonUtils.getLoginToken(), status, pageSize = 10, page = 1, orderDir = "desc")
                        .subscribeOn(baseScheduler.io())
                        .observeOn(baseScheduler.ui())
                        .subscribe(
                                { res ->
    ...
    override fun getMoreAsgJobsList(page: Int, status: String, appointmentDate: String) {
        view?.showProgress()
        subscription.add(
                apiService.getMoreAsgJobsList(CommonUtils.getLoginToken(), status, appointmentDate, pageSize = 10, page = page, orderBy = "appointmentDate")
                        .subscribeOn(baseScheduler.io())
                        .observeOn(baseScheduler.ui())
                        .subscribe(
                                { res ->
